...
export default function NewsletterMeditationPopup({
  ...
  return createPortal(
    <div role="dialog" aria-modal="true" aria-label="Newsletter popup" tabIndex={-1} onKeyDown={onEsc} className="fixed inset-0 z-[9999] p-3 sm:p-4">
      <button aria-label="Close popup" onClick={dismiss} className="absolute inset-0 cursor-default bg-black/55" />
      <div className="grid min-h-[100svh] w-full place-items-center">
        <div className="relative w-[min(92vw,640px)] max-h-[92svh] overflow-y-auto [@media(orientation:landscape)_and_(max-width:950px)]:max-w-[96vw] rounded-xl bg-[#0d1d2d] text-[var(--color-cream)] ring-1 ring-white/10 shadow-[0_6px_25px_rgba(0,0,0,0.45)] hover:bg-[#102438] transition p-[2px]">
          <button onClick={dismiss} aria-label="Close" className="absolute right-2.5 top-2.5 sm:right-3 sm:top-3 inline-flex h-8 w-8 items-center justify-center border border-white/15 text-sm opacity-80 hover:opacity-100 bg-transparent z-10">✕</button>
          ...
          {!success ? (
            <div className="grid items-stretch grid-cols-[42%_58%] md:grid-cols-[220px_1fr] [@media(orientation:landscape)_and_(max-width:950px)]:grid-cols-[200px_1fr]">
              <div className="h-full w-full bg-black/20 flex items-center justify-center overflow-hidden">
                <Image src={photoSrc} alt="Dr. Juan Pablo Salerno" width={900} height={1200} quality={95} sizes="(max-width: 950px) 42vw, 220px" className="h-full w-full object-cover object-[45%] [@media(orientation:landscape)_and_(max-width:950px)]:object-[55%] md:object-center md:rounded-l-xl" />
              </div>

              <div className="p-4 sm:p-5">
                <h3 className="hidden [@media(max-width:950px)]:block font-serif text-[22px] leading-snug mb-2 opacity-90">Please accept this guided meditation as a personal gift</h3>
                <p className="hidden [@media(max-width:950px)]:block text-[14px] opacity-90">— and my invitation to join <span className="italic">Science, Soul, and a Bit of Magic</span>, my monthly newsletter.</p>

                <h3 className="block [@media(max-width:950px)]:hidden font-serif text-[26px] md:text-[30px] leading-snug mb-2 opacity-90">Please accept this guided meditation as a personal gift</h3>
                <p className="block [@media(max-width:950px)]:hidden text-[15px] md:text-[17px] opacity-90">A 5-minute reset to return to your center. If it resonates, I’d love to stay conntected through <span className="italic">Science, Soul, and a Bit of Magic</span>, my monthly newsletter.</p>

                <form onSubmit={handleSubmit} className="mt-4 space-y-3">
                  {/* ✅ Honeypot field for bots */}
                  <input type="text" name="hp" tabIndex={-1} autoComplete="off" aria-hidden="true" className="hidden" />

                  <input type="email" name="email" required placeholder="you@example.com" className="w-full rounded-md border border-white/15 bg-white/5 px-4 py-3 outline-none placeholder-white/60 focus:ring-2 focus:ring-[var(--color-gold)]/50 focus:border-[var(--color-gold)]/50 text-[15px] [@media(orientation:landscape)_and_(max-width:950px)]:py-2.5" />
                  <button type="submit" disabled={loading} className="inline-flex w-full items-center justify-center rounded-md bg-[var(--color-gold)] text-black px-4 py-3 font-semibold shadow-md hover:shadow-lg hover:-translate-y-[1px] transition disabled:opacity-80 [@media(orientation:landscape)_and_(max-width:950px)]:py-2.5">{loading ? "Sending…" : "Subscribe"}</button>
                </form>
              </div>
            </div>
          ) : (
